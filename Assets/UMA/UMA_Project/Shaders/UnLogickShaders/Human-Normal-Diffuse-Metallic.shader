...
Properties {
	...
	_Specular ("Specular", Range (0.0, 1)) = 0.078125
	_MainTex ("Base (RGB) Gloss (A)", 2D) = "white" {}
	_BumpTex ("Base (RGB) Gloss (A)", 2D) = "white" {}
...
inline half4 LightingPseudoBRDF (SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
{
	...
	c.rgb = s.Albedo * l.rgb * 2 + _SpecColor.rgb * s.Gloss * _Shininess * l.a * _LightColor0.rgb + l.rgb*s.Specular;
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	o.Gloss = bump.r * _Shininess * 40;
	o.Specular = bump.b * _Specular * 40;
	fixed3 myNormal;
